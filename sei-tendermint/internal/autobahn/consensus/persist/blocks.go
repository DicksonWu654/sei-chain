...
// getLane returns (or optionally creates) the lane WAL. allowCreate=false after
// DeleteLane so truncate-only no-ops. Do not DeleteLane while using a pointer from here.
